...
function PlayerMovement:push(vel)
    if GodMode and GodMode.settings.master_switch and GodMode.settings.invulnerable then
        return
    end
...
function PlayerMovement:on_SPOOCed(enemy_unit)
    if GodMode and GodMode.settings and GodMode.settings.master_switch and GodMode.settings.no_cloaker then
        return
    end
